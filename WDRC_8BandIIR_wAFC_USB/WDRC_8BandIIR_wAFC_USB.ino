...
const float sample_rate_Hz = 44100.0f ; //16000, 24000 or 44117.64706f (or other frequencies in the table in AudioOutputI2S_F32
...
AudioConvert_F32toI16         convertF32toI16;            //Audio via USB Soundcard needs to be INT16 datatype
AudioOutputUSB                i2s_usb_out;              //Digital audio output to the external computer
...
AudioConnection *patchCordUSB[10];
int makeAudioConnections(void) { //call this in setup() or somewhere like that
  ...
  //send the audio out (to the external computer)
  patchCord[count++] = new AudioConnection_F32(compBroadband, 0, convertF32toI16, 0); //left output
  patchCordUSB[count++] = new AudioConnection(convertF32toI16, 0, i2s_usb_out, 0); //left output
  patchCordUSB[count++] = new AudioConnection(convertF32toI16, 0, i2s_usb_out, 1); //right output
  
  //send the audio out (to the earphones connected to the Tympan)
...
//control display and myTympan. interaction
...
SerialManager serialManager(N_CHAN_MAX,expCompLim,ampSweepTester,freqSweepTester,freqSweepTester_FIR,feedbackCancel);
...
//  myTympan.inputSelect(TYMPAN_INPUT_LINE_IN); // use the microphone jack - defaults to mic bias OFF/
...
void setup() {
  ...
  AudioMemory(60); AudioMemory_F32(200,audio_settings);  //allocate Float32 audio data blocks (primary memory used for audio processing)
...
extern void incrementKnobGain(float increment_dB) { //"extern" to make it available to other files, such as myTympan.Manager.h
